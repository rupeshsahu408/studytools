...
export default function ChapterPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
          ...
          <div className="flex items-center gap-1">
            ...
            {!shareToken ? (
              <button onClick={handleCreateShare} disabled={shareLoading || !chapter.notes}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-green-600 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-40 transition-colors">
              ...
              <button onClick={handleCopyShare}
                ...
                {shareCopied ? <CheckCheck className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            )}
            {/* Publish */}
            <button onClick={() => { setPublishError(null); setShowPublishModal(true); }}
      ...
      {/* ── Chapter info strip (below header) ── */}
      ...
      {/* ── Horizontal scrollable section tab bar ── */}
      <div ref={tabScrollRef} className="fixed top-[88px] left-0 right-0 z-30 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 overflow-x-auto scrollbar-none">
      ...
      {/* ── Main Content ── */}
      {/* Mobile: top 136px (header 48px + strip 32px + tabs 56px), Desktop: top 48px + sidebar */}
      <main className="md:ml-56 pt-[136px] md:pt-12 pb-32 md:pb-8 min-h-screen">
        {genError && (
      ...
      {/* ── Doubt Chat FAB (mobile only, not when already on chat tab) ── */}
      {activeSection !== "chat" && showChatFab && (
        <button
          onClick={() => switchSection("chat")}
          className="md:hidden fixed bottom-20 right-4 z-40 w-12 h-12 bg-green-600 hover:bg-green-700 text-white rounded-full shadow-lg flex items-center justify-center transition-all active:scale-95"
          title="Open Doubt Chat"
        >
